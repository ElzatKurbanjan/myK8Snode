...
import (
	...
	//!! mychange volumevalidation "k8s.io/kubernetes/pkg/volume/validation"
	...
	for i, mount := range container.VolumeMounts {
		...
		if subPath != "" {
			...
			if subPathExists, err := mounter.ExistsPath(hostPath); err != nil {
				...
				//!! mychange if err := subpather.SafeMakeDir(subPath, volumePath, perm); err != nil {
			...
			//!! mychange hostPath, cleanupAction, err = subpather.PrepareSafeSubpath(subpath.Subpath{
...
func (kl *Kubelet) GenerateRunContainerOptions(pod *v1.Pod, container *v1.Container, podIP string) (*kubecontainer.RunContainerOptions, func(), error) {
	...
	//!! mychange mounts, cleanupAction, err := makeMounts(pod, kl.getPodDir(pod.UID), container, hostname, hostDomainName, podIP, volumes, kl.mounter, kl.subpather, opts.Envs)
